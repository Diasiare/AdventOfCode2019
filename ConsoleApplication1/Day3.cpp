...
int Day3::Part2()
{
	...
	return cabinet.ClosestIntersection();

}

vector<pair<char, int>> Day3::ParseLine(const string& desc) {
...
void Cabinet::AddLine(const vector<pair<char, int>>& line) {
	int x = 0;
	int y = 0;
	int pos = 0;
...
int Cabinet::DistanceToClosestIntersection()
{
	...
	for (auto p1 : repr) {
		...
		for (auto p2 : p1.second) {
			int y = abs(p2.first);
			int value = p2.second.size();
			if (value > 1 && (closestX + closestY) > x + y) {
				closestX = x;
				closestY = y;
			}
		}
...
int Cabinet::ClosestIntersection() {
	int closestX = INT_MAX / 2 - 1;
	int closestY = INT_MAX / 2 - 1;
	for (auto p1 : repr) {
		for (auto p2 : p1.second) {
